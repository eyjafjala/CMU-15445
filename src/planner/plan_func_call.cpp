...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (func_name == "lower") {
    if (args.size() != 1) {
      throw Exception(fmt::format("lower function should have 1 argument, not {}\n", args.size()));
    }
    return {std::make_shared<StringExpression>(args[0], StringExpressionType::Lower)};
  }
  if (func_name == "upper") {
    if (args.size() != 1) {
      throw Exception(fmt::format("upper function should have 1 argument, not {}\n", args.size()));
    }
    return {std::make_shared<StringExpression>(args[0], StringExpressionType::Upper)};
  }
  throw Exception(fmt::format("func call {} not supported in planner yet\n", func_name));
}
