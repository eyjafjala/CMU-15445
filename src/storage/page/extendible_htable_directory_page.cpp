...
void ExtendibleHTableDirectoryPage::Init(uint32_t max_depth) {
  ...
  global_depth_ = 0;
  for(uint32_t i = 0, sz = MaxSize(); i < sz; i ++) {
    local_depths_[i] = 0;
...
auto ExtendibleHTableDirectoryPage::GetSplitImageIndex(uint32_t bucket_idx) const -> uint32_t { 
  ...
  return local_idx ^ (1 << GetLocalDepth(bucket_idx) - 1); //这里的镜像是基于local的，目前理解不是基于dir去找bucket层面的，而是bucket层面的。
}
...
void ExtendibleHTableDirectoryPage::IncrGlobalDepth() {
  ...
  for(uint32_t i = 0, sz = Size() >> 1; i < sz; i ++) {
    local_depths_[i + sz] = local_depths_[i];
    bucket_page_ids_[i + sz] = bucket_page_ids_[i];
  }
}
